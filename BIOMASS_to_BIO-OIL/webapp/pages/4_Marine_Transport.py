...
st.markdown(
    ...
    "### **Voyage:** #" \
    "\n\n A container ship carrying 100,000 tonnes of goods travels 20,000 kilometers from China to Denmark to supply the danish consumers. A single ship serves many ports, though — so we count only the share actually bound for Denmark (about **30,000 tonnes** by default). "
...
# ---------------------------------------------------------------------------
total_cargo = meta.DEFAULT_TONNES
cargo_to_dk = meta.DEFAULT_DK_TONNES
distance = meta.DEFAULT_DISTANCE_KM
intensity = meta.MJ_PER_TKM_DEFAULT
hfo_lhv = meta.HFO_LHV_DEFAULT
hfo_gwp = meta.HFO_GWP_DEFAULT
feedstock_tonnes = meta.DEFAULT_FEEDSTOCK_TONNES
origin = meta.DEFAULT_ORIGIN
destination = meta.DEFAULT_DESTINATION

# ---------------------------------------------------------------------------
# Sidebar — biomass filter only
# ---------------------------------------------------------------------------
...
scale = mj_fuel
basis_note = "for the Denmark-bound goods"
...
# ---------------------------------------------------------------------------
# Voyage illustration (hero) — energy/yield basis
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Advanced scenario analysis + data table (merged expander)
# ---------------------------------------------------------------------------
with st.expander("Advanced scenario analysis & data table"):
    all_categories = sorted(lca_df["category"].unique())

    _PRESET_TOP4 = "⭐ Top 4 categories"
    _PRESET_ALL  = "✅ All categories"

    # Resolve any preset selected on the previous run before the widget renders
    if "mt_cat_raw" not in st.session_state:
        st.session_state["mt_cat_raw"] = meta.default_categories(all_categories)
    elif _PRESET_ALL in st.session_state["mt_cat_raw"]:
        st.session_state["mt_cat_raw"] = all_categories
    elif _PRESET_TOP4 in st.session_state["mt_cat_raw"]:
        st.session_state["mt_cat_raw"] = meta.default_categories(all_categories)

    _raw = st.multiselect(
        "Impact categories",
        options=[_PRESET_TOP4, _PRESET_ALL] + all_categories,
        key="mt_cat_raw",
        help="Pick individual categories, or choose a preset from the top of the list.",
    )
    selected_categories = [c for c in _raw if c not in (_PRESET_TOP4, _PRESET_ALL)]

    if not selected_categories:
        st.info("Select at least one impact category above.")
    else:
        st.caption(
        ...
        def _bar(cat: str) -> go.Figure:
            ...
            fig = px.bar(
                ...
                labels={"value": unit, "biomass_type": "Biomass", "yield_class": "Bio-oil yield"},
                template="plotly_white",
                custom_data=["yield_class", "oil_yield_kg_per_kg"],
            )
            fig.update_traces(
                hovertemplate="<b>%{y}</b><br>%{x:.4g} " + unit
        ...
        tbl = work[work["category"].isin(selected_categories)][
            ["biomass_type", "yield_class", "oil_yield_kg_per_kg", "category", "unit", "score", "value"]
        ].copy()
    ...
    st.markdown(
        ...
        **Assumptions & caveats**
        - Energy intensity ({intensity:.4f} MJ/t·km) and the HFO benchmark are **external reference
          values** from literature — verify against the ecoinvent container-ship LCI.
